...
function calculateConfidence(match: RegExpMatchArray, text: string, techName: string): number {
  ...
  const context = text
    .slice(Math.max(0, (match.index || 0) - 200), (match.index || 0) + 200)
    .toLowerCase();
  if (
    context.includes("requirements") ||
    context.includes("qualifications") ||
    context.includes("experience with")
  ) {
    confidence += 10;
  }
 ...
 */
export async function extractTechnologiesFromJob(
  jobId: number,
  ...
  const techs = await db.select().from(technologies).where(eq(technologies.visible, true));
 ...
 */
export async function extractTechnologiesForCompany(
  companyId: number,
): Promise<{ jobs: number; mentions: number }> {
 ...
 */
export async function extractTechnologiesFromAllJobs(): Promise<{
  jobs: number;
  mentions: number;
}> {
  const allJobs = await db.select({ id: jobs.id }).from(jobs).where(eq(jobs.status, "active"));
...
export async function clearTechMentionsForJob(jobId: number) {
  await db.delete(jobTechnologyMentions).where(eq(jobTechnologyMentions.jobId, jobId));
}
...
export async function extractTechnologiesForSource(
  sourceId: number,
): Promise<{ jobs: number; mentions: number }> {
  const sourceJobs = await db
    ...
    .from(jobs)
    .where(
      and(eq(jobs.sourceId, sourceId), inArray(jobs.status, [...SOURCE_TECH_EXTRACTION_STATUSES])),
    );
