...
export async function generateEducationSlug(name: string, excludeId?: number): Promise<string> {
  ...
  if (excludeId) {
    const current = await db.select({ slug: education.slug }).from(education).where(eq(education.id, excludeId)).get();
    if (current) {
...
export async function createEducation(item: Omit<NewEducation, "slug">): Promise<Education> {
  ...
  const [newItem] = await db.insert(education).values({ ...item, slug }).returning();
...
export async function updateEducation(id: number, item: Partial<Omit<NewEducation, "slug">>): Promise<Education | null> {
...
export async function getAllEducation(includeHidden: boolean = false): Promise<Education[]> {
  ...
  return db.select().from(education).where(eq(education.visible, true)).orderBy(desc(education.createdAt));
}
...
export async function convertCompanyToEducation(
  ...
  const [newInstitution] = await db.insert(education).values({
    slug,
    name: company.name,
