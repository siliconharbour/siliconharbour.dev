...
const EXECUTE_DESCRIPTION = [
  ...
  "createEvent creates a one-time event tied to a single external link (use this for LinkedIn event URLs or other sources we can't scrape — pair with a browser tool to read the event page first). Dates are YYYY-MM-DD, times are HH:mm local time in America/St_Johns. Omit endDate for same-day events.",
  "getManualEvents() returns all events not tied to an import source (i.e. created via createEvent or the admin /manage/events UI).",
