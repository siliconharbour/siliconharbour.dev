...
The `execute` tool (for triggering import syncs and reviewing pending items) requires a Bearer token provided by the site owner. Authenticated access:
...
export default { upcoming: upcoming.length, companies: cos.map(c => c.name), groups: grps.map(g => g.name) }
```
